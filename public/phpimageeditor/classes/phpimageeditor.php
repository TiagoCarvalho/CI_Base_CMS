	...
	class PHPImageEditor 
	{
		...
		function Action($actionType)
		{
			...
			if ($actionType == $this->actionSaveAndClose)
			{
				...
				PIE_Echo("<script language='javascript' type='text/javascript'>  $reloadParentBrowser 
				pathArray = parent.location.pathname.split('/');
				newPathname = window.location.protocol + '//' + window.location.host;
				for ( i = 0; i <= 4; i++ ) {
				newPathname += pathArray[i];
				newPathname += '/';
				}
				window.open(newPathname,'_parent','');window.close();</script>");
			}
		}
